...
//===---------------------- NxKernelOps.cpp - NxKernel Operations -----------------===//
//
// Copyright 2025 Intel Labs.
//
// =============================================================================
//
// This file contains implementations of nx operations.
//
//===----------------------------------------------------------------------===//
...
#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelOps.hpp"
...
void NxKernelDialect::initialize() {
  // addTypes<nx::LoopType, nx::StringType>();
  addOperations<
#define GET_OP_LIST
#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelOps.cpp.inc"
      >();
}
Type NxKernelDialect::parseType(DialectAsmParser &parser) const {
  // StringRef keyword;
  ...
  // if (keyword == "loop")
  //   return nx::LoopType::get(context);
  // if (keyword == "string")
  //   return nx::StringType::get(context);

  // parser.emitError(parser.getNameLoc(), "unknown nx type: " + keyword);
  return Type();
}

void NxKernelDialect::printType(Type type, DialectAsmPrinter &os) const {
  // TypeSwitch<Type>(type)
  //     .Case<nx::LoopType>([&](Type) { os << "loop"; })
  //     .Case<nx::StringType>([&](Type) { os << "string"; })
  //     .Default([](Type) { llvm_unreachable("Unexpected 'nx' type kind"); });
}
...
#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelTypes.cpp.inc"

#define GET_ATTRDEF_CLASSES
#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelAttributes.cpp.inc"
#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelEnums.cpp.inc"

#include "src/Accelerators/NX/Dialect/NxKernel/NxKernelDialect.cpp.inc"
...
// // void NxKernelCallOp::build(OpBuilder &builder, ::mlir::OperationState &odsState,
// //     std::string funcNameStr, ValueRange resultVals, Operation *op,
// //     ValueRange operands, bool copyAttrs) {
// //   // Creates parameters for NxKernelCall for Optional input (with NoneType)
// //   // The semantics of optional input is ONNX Op specific and should be
// //   // handled when lowering ONNX Op, not lowering NxKernelCall.
...
// // void NxKernelCallOp::getEffects(
...
// //===----------------------------------------------------------------------===//
// // NxKernelDefineLoopsOp
// //===----------------------------------------------------------------------===//

// void NxKernelDefineLoopsOp::build(
...
//   result.types.append(num_loops, nx::LoopType::get(builder.getContext()));
//   result.addAttribute(
//       getNumLoopsAttrName(), builder.getI64IntegerAttr(num_loops));
// }

// void NxKernelDefineLoopsOp::print(OpAsmPrinter &printer) {
//   auto numLoopAttr = (*this)->getAttrOfType<IntegerAttr>(
//       NxKernelDefineLoopsOp::getNumLoopsAttrName());
//   printer << ' ' << numLoopAttr.getValue().getSExtValue();
// }

// ParseResult NxKernelDefineLoopsOp::parse(
...
//           NxKernelDefineLoopsOp::getNumLoopsAttrName(), result.attributes))
//     return failure();

//   auto loopTypes =
//       llvm::SmallVector<Type, 4>(numLoops.getValue().getSExtValue(),
//           nx::LoopType::get(builder.getContext()));
...
// /*!
//  * Build a NxKernel Dialect iterate operation.
//  * input_loops: a collection of input nx.loops being optimized.
//  * optimized_loops: a collection of optimized (scheduled) nx.loops.
...
//  */
// void NxKernelIterateOp::build(OpBuilder &builder, OperationState &result,
//     nx::NxKernelIterateOperandPack operandPack, ValueRange iterArgInits,
...
//   result.addAttribute(
//       NxKernelIterateOp::getBoundsAttrName(), operandPack.getAttributes());
...
//   // associated with the same nx.iterate operation.
...
//   // TODO: May want to change NxKernelIterateOperandPack to use ValueRanges...
...
//   nx::NxKernelIterateOperandPack pack(builder, origLoops, optLoops);
...
// MutableArrayRef<OpOperand> NxKernelIterateOp::getInitsMutable() {
...
// NxKernelIterateOp::getYieldedValuesMutable() {
//   return cast<NxKernelYieldOp>(getBody()->getTerminator()).getOperandsMutable();
// }

// void NxKernelIterateOp::print(OpAsmPrinter &printer) {
//   printer << "(";
...
//   auto boundItr =
//       (*this)
//           ->getAttrOfType<ArrayAttr>(NxKernelIterateOp::getBoundsAttrName())
//           .getValue()
//           .begin();
...
// LogicalResult NxKernelYieldOp::verify() {
//   auto *parentOp = (*this)->getParentOp();
//   auto results = parentOp->getResults();
//   auto operands = getOperands();

//   if (!isa<NxKernelIterateOp>(parentOp))
//     return emitOpError() << "only terminates nx.iterate regions";
...
// ParseResult NxKernelIterateOp::parse(OpAsmParser &parser, OperationState &result) {
//   auto builder = parser.getBuilder();

//   Type loopType = nx::LoopType::get(result.getContext());
...
//   result.addAttribute(NxKernelIterateOp::getNumOptimizedLoopsAttrName(),
...
//   result.addAttribute(NxKernelIterateOp::getBoundsAttrName(),
...
//   // Ensure iterate region is closed off with nx.terminate.
//   NxKernelIterateOp::ensureTerminator(
//       *region, parser.getBuilder(), result.location);

//   return success();
// }

// ::llvm::SmallVector<Region *> NxKernelIterateOp::getLoopRegions() {
//   return {&getBodyRegion()};
// }

// LogicalResult NxKernelIterateOp::verify() {
//   // TODO: Verify number of induction variable bounds matches the number of
//   // input loops.
//   return success();
// }

// void NxKernelRegionOp::build(OpBuilder &builder, OperationState &result,
...
// //===----------------------------------------------------------------------===//
// // NxKernelEntryPointOp
// //===----------------------------------------------------------------------===//

// void NxKernelEntryPointOp::build(mlir::OpBuilder &builder, OperationState &state,
//     SymbolRefAttr funcAttr, IntegerAttr numInputs, IntegerAttr numOutputs,
//     StringAttr signature) {
//   state.addAttribute(NxKernelEntryPointOp::getEntryPointFuncAttrName(), funcAttr);
//   state.addAttribute(NxKernelEntryPointOp::getNumInputsAttrName(), numInputs);
//   state.addAttribute(NxKernelEntryPointOp::getNumOutputsAttrName(), numOutputs);
//   state.addAttribute(NxKernelEntryPointOp::getSignatureAttrName(), signature);
// }

// void NxKernelInstrumentOp::build(mlir::OpBuilder &builder, OperationState &state,
...
// void NxKernelPermuteOp::build(::mlir::OpBuilder &odsBuilder,
...
// //===----------------------------------------------------------------------===//
// // NxKernelGetInductionVariableValueOp
// //===----------------------------------------------------------------------===//

// void NxKernelGetInductionVariableValueOp::build(::mlir::OpBuilder &odsBuilder,
//     ::mlir::OperationState &odsState, ValueRange odsLoops) {
...
// // only 1D vectors.
// void NxKernelVectorTypeCastOp::build(OpBuilder &builder, OperationState &state,
...
// bool NxKernelVectorTypeCastOp::areCastCompatible(
...
// OpFoldResult NxKernelVectorTypeCastOp::fold(FoldAdaptor adaptor) {
//   if (OpFoldResult folded = OpFoldResult())
//     return folded;
//   return succeeded(foldMemRefCast(*this)) ? getResult() : Value();
// }

// MutableOperandRange NxKernelSpecializedKernel::getLoopRefs() {
...
// void NxKernelMatMulOp::build(::mlir::OpBuilder &odsBuilder,
...
// LogicalResult NxKernelMatMulOp::verify() {
//   NxKernelMatMulOpAdaptor operandAdaptor = NxKernelMatMulOpAdaptor(*this);
//   uint64_t aRank =
...
// MutableOperandRange NxKernelMatMulOp::getLoopRefs() { return getLoopsMutable(); }

// //===----------------------------------------------------------------------===//
// // NxKernelCopyToBufferOp
// //===----------------------------------------------------------------------===//

// void NxKernelCopyToBufferOp::build(::mlir::OpBuilder &odsBuilder,
...
// LogicalResult NxKernelCopyToBufferOp::verify() {
//   NxKernelCopyToBufferOpAdaptor opAdaptor = NxKernelCopyToBufferOpAdaptor(*this);
...
// //===----------------------------------------------------------------------===//
// // NxKernelCopyFromBufferOp
// //===----------------------------------------------------------------------===//

// void NxKernelCopyFromBufferOp::build(::mlir::OpBuilder &odsBuilder,
//     ::mlir::OperationState &odsState, Value odsBufferMemref, Value odsMemref,
...
// LogicalResult NxKernelCopyFromBufferOp::verify() {
//   NxKernelCopyFromBufferOpAdaptor opAdaptor = NxKernelCopyFromBufferOpAdaptor(*this);
//   IndexExprBuilderForAnalysis createIE(getLoc());
//   int64_t bufferRank = createIE.getShapedTypeRank(opAdaptor.getBuffer());
//   int64_t destRank =
...
// void NxKernelSeqExtractOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// void NxKernelSeqStoreOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// std::optional<Operation *> NxKernelSeqExtractOp::buildDealloc(
...
// std::optional<Value> NxKernelSeqExtractOp::buildClone(
//     OpBuilder &builder, Value alloc) {
//   return builder.create<bufferization::CloneOp>(alloc.getLoc(), alloc)
//       .getResult();
// }

// void NxKernelSeqAllocOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// std::optional<Operation *> NxKernelSeqAllocOp::buildDealloc(
//     OpBuilder &builder, Value alloc) {
//   Location loc = alloc.getLoc();
//   // MultiDialectBuilder<NxKernelBuilder> create(builder, loc);
//   return builder.create<NxKernelSeqDeallocOp>(loc, alloc).getOperation();
// }

// std::optional<Value> NxKernelSeqAllocOp::buildClone(
//     OpBuilder &builder, Value alloc) {
//   return builder.create<bufferization::CloneOp>(alloc.getLoc(), alloc)
//       .getResult();
// }
...
// //===----------------------------------------------------------------------===//
// // NxKernelGetLinearOffsetIndexOp
// //===----------------------------------------------------------------------===//

// void NxKernelGetLinearOffsetIndexOp::build(OpBuilder &builder,
...
// ParseResult NxKernelGetLinearOffsetIndexOp::parse(
//     OpAsmParser &parser, OperationState &result) {
//   auto &builder = parser.getBuilder();
...
//           NxKernelGetLinearOffsetIndexOp::getMapAttrStrName(), result.attributes) ||
...
// void NxKernelGetLinearOffsetIndexOp::print(OpAsmPrinter &p) {
//   p << " " << getMemRef() << " at [";
...
// void NxKernelPrefetchOp::build(OpBuilder &builder, OperationState &result,
...
// //
// // nx.prefetch %0[%i, %j + 5], read, locality<3>, data : memref<400x400xi32>
// // Code lifted from affine prefetch as is.
// // I have seen parsing errors when multiple '#x' are used in the indices,
// // could not tell why.
// //   nx.prefetch %arg0[%1#0, %1#1, %3], read, locality<3>, data :
// //     memref<8x256x512xf32>
// // With only one, it works.
// //

// ParseResult NxKernelPrefetchOp::parse(OpAsmParser &parser, OperationState &result) {
//   auto &builder = parser.getBuilder();
//   auto indexTy = builder.getIndexType();
...
//       parser.parseAffineMapOfSSAIds(mapOperands, mapAttr,
//           NxKernelPrefetchOp::getMapAttrStrName(), result.attributes) ||
//       parser.parseComma() || parser.parseKeyword(&readOrWrite) ||
//       parser.parseComma() || parser.parseKeyword("locality") ||
//       parser.parseLess() ||
//       parser.parseAttribute(hintInfo, i32Type,
//           NxKernelPrefetchOp::getLocalityHintAttrStrName(), result.attributes) ||
...
//   result.addAttribute(NxKernelPrefetchOp::getIsWriteAttrStrName(),
...
//   result.addAttribute(NxKernelPrefetchOp::getIsDataCacheAttrStrName(),
//       parser.getBuilder().getBoolAttr(cacheType == "data"));

//   return success();
// }

// void NxKernelPrefetchOp::print(OpAsmPrinter &p) {
//   p << " " << getMemref() << '[';
//   AffineMapAttr mapAttr =
//       (*this)->getAttrOfType<AffineMapAttr>(getMapAttrStrName());
//   if (mapAttr)
...
// void NxKernelMemcpyOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// void NxKernelMemsetOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
//   effects.emplace_back(MemoryEffects::Write::get(), &getDestMutable(),
//       SideEffects::DefaultResource::get());
