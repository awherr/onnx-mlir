...
// =============================================================================
//
// This file declares the Nx Dialect Builder.
//
//===----------------------------------------------------------------------===//

#ifndef ONNX_MLIR_DIALECT_BUILDER_DEN_H
#define ONNX_MLIR_DIALECT_BUILDER_DEN_H

#include "src/Dialect/Nx/NxOps.hpp"
#include "src/Dialect/Mlir/DialectBuilder.hpp"
#include "src/Dialect/Mlir/IndexExprBuilder.hpp"

namespace onnx_mlir {

//====-------------------- Support for Nx Builder ----------------------===//

struct NxBuilder : public DialectBuilder {
  NxBuilder(mlir::Location loc) : DialectBuilder(loc) {}
  NxBuilder(mlir::OpBuilder &b, mlir::Location loc)
      : DialectBuilder(b, loc) {}
  NxBuilder(const DialectBuilder &db) : DialectBuilder(db) {}
  virtual ~NxBuilder() {}

  // Common load/store interface (nx/affine/memref)
  ...
  void region(
      mlir::function_ref<void(const NxKernelBuilder &createNxKernel)> bodyBuilderFn)
      const;
  ...
  // function implement the body of the loop, and receive a DEN builder and the
  // loop indices.
  using NxKernelLoopBodyFn = impl::LoopBodyFn<NxKernelBuilder>;
  using NxKernelLoopBody2Fn = mlir::function_ref<void(
      const NxKernelBuilder &, mlir::ValueRange, mlir::ValueRange)>;

  void iterate(mlir::ValueRange originalLoops, mlir::ValueRange optimizedLoops,
      mlir::ValueRange lbs, mlir::ValueRange ubs,
      NxKernelLoopBodyFn bodyBuilderFn) const;
  // Deprecated.
  mlir::NxKernelIterateOp iterate(mlir::ValueRange originalLoops,
      mlir::ValueRange optimizedLoops, mlir::ValueRange lbs,
      mlir::ValueRange ubs, mlir::ValueRange inits,
      NxKernelLoopBody2Fn bodyBuilderFn) const;

  mlir::NxKernelIterateOp iterate(
      const nx::NxKernelIterateOperandPack &operands) const;
  ...
  void iterateIE(mlir::ValueRange originalLoops,
      ...
      mlir::ArrayRef<IndexExpr> ubs, NxKernelLoopBodyFn bodyBuilderFn) const;
  // Deprecated.
  mlir::NxKernelIterateOp iterateIE(mlir::ValueRange originalLoops,
      mlir::ValueRange optimizedLoops, mlir::ArrayRef<IndexExpr> lbs,
      mlir::ArrayRef<IndexExpr> ubs, mlir::ValueRange inits,
      NxKernelLoopBody2Fn bodyBuilderFn) const;

  // Common loop interface (nx/affine/scf).
  void forLoopIE(IndexExpr lb, IndexExpr ub, int64_t step, bool useParallel,
      NxKernelLoopBodyFn builderFn) const;
  ...
  void forExplicitParallelLoopIE(IndexExpr lb, IndexExpr ub,
      IndexExpr threadNum, NxKernelLoopBodyFn builderFn) const;

  // Common simd loop interface (nx/affine/scf).
  /*
     ...
     The loop body is constructed by calling each of the NxKernelSimdIterateBodyFn
     ...
     To generate their output, each NxKernelSimdIterateBodyFn function is given
  ...
  using NxKernelSimdIterateBodyFn = impl::SimdIterateBodyFn<NxKernelBuilder>;
  void simdIterateIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      mlir::ArrayRef<NxKernelSimdIterateBodyFn> bodyBuilderFnList) const;
  ...
  /*
     ...
     here.
    */
  using NxKernelSimdReductionBodyFn = impl::SimdReductionBodyFn<NxKernelBuilder>;
  using NxKernelSimdPostReductionBodyFn =
      impl::SimdPostReductionBodyFn<NxKernelBuilder>;
  ...
  void simdReduceIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      mlir::ArrayRef<NxKernelSimdReductionBodyFn> reductionBodyFnList,
      /* post reduction function (simd to scalar + post processing)*/
      mlir::ArrayRef<NxKernelSimdPostReductionBodyFn> postReductionBodyFnList)
      const;
  ...
  */
  void simdReduce2DIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      NxKernelSimdReductionBodyFn reductionBodyFn,
      /* post reduction functions (post processing ONLY)*/
      NxKernelSimdPostReductionBodyFn postReductionBodyFn) const;
  ...
  void matmul(
      ...
      // Loops are the nx loop indices that this matmul replaces
  ...
  mlir::NxKernelMovableOp movable() const;
...
//====--- Support for Affine Builder with NxKernel Mem Ops ------------------===//

// We use here a Affine builder that generates NxKernel Load and Store ops instead
// of the affine memory ops directly. This is because we can still generate
// NxKernel Ops while lowering the dialect, and the big advantage of the NxKernel
// memory operations is that they distinguish themselves if they are affine or
// not.
using AffineBuilderNxKernelMem =
    GenericAffineBuilder<mlir::NxKernelLoadOp, mlir::NxKernelStoreOp>;

// =============================================================================
// IndexExpr Builder for building
// =============================================================================

struct IndexExprBuilderForNxKernel : IndexExprBuilder {
  IndexExprBuilderForNxKernel(mlir::Location loc) : IndexExprBuilder(loc) {}
  IndexExprBuilderForNxKernel(mlir::OpBuilder &b, mlir::Location loc)
      : IndexExprBuilder(b, loc) {}
  IndexExprBuilderForNxKernel(const DialectBuilder &db) : IndexExprBuilder(db) {}
  virtual ~IndexExprBuilderForNxKernel() {}
...
// =============================================================================
// MultiDialectBuilder for NxKernel
// =============================================================================

// Recursive class specialized for AffineBuilderNxKernelMem refereed to as
// affineKMem.
template <class... Ts>
struct MultiDialectBuilder<AffineBuilderNxKernelMem, Ts...>
  ...
  AffineBuilderNxKernelMem affineKMem;
};

// Recursive class specialized for NxKernelBuilder referred to as nx.
template <class... Ts>
struct MultiDialectBuilder<NxKernelBuilder, Ts...> : MultiDialectBuilder<Ts...> {
  MultiDialectBuilder(mlir::OpBuilder &b, mlir::Location loc)
      : MultiDialectBuilder<Ts...>(b, loc), nx(b, loc) {}
  MultiDialectBuilder(const DialectBuilder &db)
      : MultiDialectBuilder<Ts...>(db), nx(db) {}
  NxKernelBuilder nx;
};

// Recursive class specialized for IndexExprBuilderForNxKernel referred to as
// denIE.
template <class... Ts>
struct MultiDialectBuilder<IndexExprBuilderForNxKernel, Ts...>
    : MultiDialectBuilder<Ts...> {
  MultiDialectBuilder(mlir::OpBuilder &b, mlir::Location loc)
      : MultiDialectBuilder<Ts...>(b, loc), denIE(b, loc) {}
  MultiDialectBuilder(const DialectBuilder &db)
      : MultiDialectBuilder<Ts...>(db), denIE(db) {}
  IndexExprBuilderForNxKernel denIE;
};
