...
#include "src/Accelerators/NX/Dialect/Nx/NxOps.hpp"
...
void NxDialect::initialize() {
  // addTypes<nx::LoopType, nx::StringType>();
  addOperations<
#define GET_OP_LIST
#include "src/Accelerators/NX/Dialect/Nx/NxOps.cpp.inc"
      >();
}
Type NxDialect::parseType(DialectAsmParser &parser) const {
  StringRef keyword;
...
void NxDialect::printType(Type type, DialectAsmPrinter &os) const {
...
#include "src/Accelerators/NX/Dialect/Nx/NxDialect.cpp.inc"
...
// // void NxCallOp::build(OpBuilder &builder, ::mlir::OperationState &odsState,
// //     std::string funcNameStr, ValueRange resultVals, Operation *op,
// //     ValueRange operands, bool copyAttrs) {
// //   // Creates parameters for NxCall for Optional input (with NoneType)
// //   // The semantics of optional input is ONNX Op specific and should be
// //   // handled when lowering ONNX Op, not lowering NxCall.
...
// void NxDefineLoopsOp::print(OpAsmPrinter &printer) {
//   auto numLoopAttr = (*this)->getAttrOfType<IntegerAttr>(
//       NxDefineLoopsOp::getNumLoopsAttrName());
//   printer << ' ' << numLoopAttr.getValue().getSExtValue();
// }

// ParseResult NxDefineLoopsOp::parse(
...
//           NxDefineLoopsOp::getNumLoopsAttrName(), result.attributes))
//     return failure();
...
// /*!
//  * Build a Nx Dialect iterate operation.
...
//  */
// void NxIterateOp::build(OpBuilder &builder, OperationState &result,
//     nx::NxIterateOperandPack operandPack, ValueRange iterArgInits,
...
//   result.addAttribute(
//       NxIterateOp::getBoundsAttrName(), operandPack.getAttributes());
...
//   // TODO: May want to change NxIterateOperandPack to use ValueRanges...
...
//   nx::NxIterateOperandPack pack(builder, origLoops, optLoops);
...
// MutableArrayRef<OpOperand> NxIterateOp::getInitsMutable() {
...
// NxIterateOp::getYieldedValuesMutable() {
//   return cast<NxYieldOp>(getBody()->getTerminator()).getOperandsMutable();
// }

// void NxIterateOp::print(OpAsmPrinter &printer) {
//   printer << "(";
...
//   auto boundItr =
//       (*this)
//           ->getAttrOfType<ArrayAttr>(NxIterateOp::getBoundsAttrName())
//           .getValue()
//           .begin();
...
// LogicalResult NxYieldOp::verify() {
//   auto *parentOp = (*this)->getParentOp();
//   auto results = parentOp->getResults();
//   auto operands = getOperands();

//   if (!isa<NxIterateOp>(parentOp))
...
// ParseResult NxIterateOp::parse(OpAsmParser &parser, OperationState &result) {
//   auto builder = parser.getBuilder();
...
//   result.addAttribute(NxIterateOp::getNumOptimizedLoopsAttrName(),
...
//   result.addAttribute(NxIterateOp::getBoundsAttrName(),
...
//   NxIterateOp::ensureTerminator(
//       *region, parser.getBuilder(), result.location);

//   return success();
// }

// ::llvm::SmallVector<Region *> NxIterateOp::getLoopRegions() {
//   return {&getBodyRegion()};
// }

// LogicalResult NxIterateOp::verify() {
//   // TODO: Verify number of induction variable bounds matches the number of
//   // input loops.
//   return success();
// }

// void NxRegionOp::build(OpBuilder &builder, OperationState &result,
...
// void NxEntryPointOp::build(mlir::OpBuilder &builder, OperationState &state,
//     SymbolRefAttr funcAttr, IntegerAttr numInputs, IntegerAttr numOutputs,
//     StringAttr signature) {
//   state.addAttribute(NxEntryPointOp::getEntryPointFuncAttrName(), funcAttr);
//   state.addAttribute(NxEntryPointOp::getNumInputsAttrName(), numInputs);
//   state.addAttribute(NxEntryPointOp::getNumOutputsAttrName(), numOutputs);
//   state.addAttribute(NxEntryPointOp::getSignatureAttrName(), signature);
// }

// void NxInstrumentOp::build(mlir::OpBuilder &builder, OperationState &state,
...
// void NxBlockOp::build(::mlir::OpBuilder &odsBuilder,
...
// void NxPermuteOp::build(::mlir::OpBuilder &odsBuilder,
...
// //===----------------------------------------------------------------------===//
// // NxGetInductionVariableValueOp
// //===----------------------------------------------------------------------===//

// void NxGetInductionVariableValueOp::build(::mlir::OpBuilder &odsBuilder,
//     ::mlir::OperationState &odsState, ValueRange odsLoops) {
...
// // only 1D vectors.
// void NxVectorTypeCastOp::build(OpBuilder &builder, OperationState &state,
...
// bool NxVectorTypeCastOp::areCastCompatible(
...
// OpFoldResult NxVectorTypeCastOp::fold(FoldAdaptor adaptor) {
//   if (OpFoldResult folded = OpFoldResult())
//     return folded;
//   return succeeded(foldMemRefCast(*this)) ? getResult() : Value();
// }

// MutableOperandRange NxSpecializedKernel::getLoopRefs() {
...
// void NxMatMulOp::build(::mlir::OpBuilder &odsBuilder,
...
// LogicalResult NxMatMulOp::verify() {
//   NxMatMulOpAdaptor operandAdaptor = NxMatMulOpAdaptor(*this);
//   uint64_t aRank =
...
// MutableOperandRange NxMatMulOp::getLoopRefs() { return getLoopsMutable(); }

// //===----------------------------------------------------------------------===//
// // NxCopyToBufferOp
// //===----------------------------------------------------------------------===//

// void NxCopyToBufferOp::build(::mlir::OpBuilder &odsBuilder,
...
// LogicalResult NxCopyToBufferOp::verify() {
//   NxCopyToBufferOpAdaptor opAdaptor = NxCopyToBufferOpAdaptor(*this);
...
// void NxCopyFromBufferOp::build(::mlir::OpBuilder &odsBuilder,
//     ::mlir::OperationState &odsState, Value odsBufferMemref, Value odsMemref,
...
// LogicalResult NxCopyFromBufferOp::verify() {
//   NxCopyFromBufferOpAdaptor opAdaptor = NxCopyFromBufferOpAdaptor(*this);
//   IndexExprBuilderForAnalysis createIE(getLoc());
//   int64_t bufferRank = createIE.getShapedTypeRank(opAdaptor.getBuffer());
//   int64_t destRank =
...
// void NxSeqExtractOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// std::optional<Operation *> NxSeqExtractOp::buildDealloc(
...
// std::optional<Value> NxSeqExtractOp::buildClone(
//     OpBuilder &builder, Value alloc) {
//   return builder.create<bufferization::CloneOp>(alloc.getLoc(), alloc)
//       .getResult();
// }

// void NxSeqAllocOp::getEffects(
//     SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
//         &effects) {
...
// std::optional<Operation *> NxSeqAllocOp::buildDealloc(
//     OpBuilder &builder, Value alloc) {
//   Location loc = alloc.getLoc();
//   // MultiDialectBuilder<NxBuilder> create(builder, loc);
//   return builder.create<NxSeqDeallocOp>(loc, alloc).getOperation();
// }

// std::optional<Value> NxSeqAllocOp::buildClone(
//     OpBuilder &builder, Value alloc) {
//   return builder.create<bufferization::CloneOp>(alloc.getLoc(), alloc)
//       .getResult();
// }
...
// //===----------------------------------------------------------------------===//
// // NxGetLinearOffsetIndexOp
// //===----------------------------------------------------------------------===//

// void NxGetLinearOffsetIndexOp::build(OpBuilder &builder,
...
// ParseResult NxGetLinearOffsetIndexOp::parse(
//     OpAsmParser &parser, OperationState &result) {
//   auto &builder = parser.getBuilder();
...
//           NxGetLinearOffsetIndexOp::getMapAttrStrName(), result.attributes) ||
...
// void NxGetLinearOffsetIndexOp::print(OpAsmPrinter &p) {
//   p << " " << getMemRef() << " at [";
...
// void NxPrefetchOp::build(OpBuilder &builder, OperationState &result,
...
// ParseResult NxPrefetchOp::parse(OpAsmParser &parser, OperationState &result) {
//   auto &builder = parser.getBuilder();
//   auto indexTy = builder.getIndexType();
...
//       parser.parseAffineMapOfSSAIds(mapOperands, mapAttr,
//           NxPrefetchOp::getMapAttrStrName(), result.attributes) ||
//       parser.parseComma() || parser.parseKeyword(&readOrWrite) ||
//       parser.parseComma() || parser.parseKeyword("locality") ||
//       parser.parseLess() ||
//       parser.parseAttribute(hintInfo, i32Type,
//           NxPrefetchOp::getLocalityHintAttrStrName(), result.attributes) ||
...
//   result.addAttribute(NxPrefetchOp::getIsWriteAttrStrName(),
...
//   result.addAttribute(NxPrefetchOp::getIsDataCacheAttrStrName(),
//       parser.getBuilder().getBoolAttr(cacheType == "data"));

//   return success();
// }

// void NxPrefetchOp::print(OpAsmPrinter &p) {
//   p << " " << getMemref() << '[';
//   AffineMapAttr mapAttr =
//       (*this)->getAttrOfType<AffineMapAttr>(getMapAttrStrName());
//   if (mapAttr)
