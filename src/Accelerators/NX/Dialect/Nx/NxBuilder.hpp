...
struct NxBuilder : public DialectBuilder {
  ...
  void region(
      mlir::function_ref<void(const NxBuilder &createNx)> bodyBuilderFn)
      const;
  ...
  // loop indices.
  using NxLoopBodyFn = impl::LoopBodyFn<NxBuilder>;
  using NxLoopBody2Fn = mlir::function_ref<void(
      const NxBuilder &, mlir::ValueRange, mlir::ValueRange)>;

  void iterate(mlir::ValueRange originalLoops, mlir::ValueRange optimizedLoops,
      mlir::ValueRange lbs, mlir::ValueRange ubs,
      NxLoopBodyFn bodyBuilderFn) const;
  // Deprecated.
  mlir::NxIterateOp iterate(mlir::ValueRange originalLoops,
      mlir::ValueRange optimizedLoops, mlir::ValueRange lbs,
      mlir::ValueRange ubs, mlir::ValueRange inits,
      NxLoopBody2Fn bodyBuilderFn) const;

  mlir::NxIterateOp iterate(
      const nx::NxIterateOperandPack &operands) const;
  ...
  void iterateIE(mlir::ValueRange originalLoops,
      ...
      mlir::ArrayRef<IndexExpr> ubs, NxLoopBodyFn bodyBuilderFn) const;
  // Deprecated.
  mlir::NxIterateOp iterateIE(mlir::ValueRange originalLoops,
      mlir::ValueRange optimizedLoops, mlir::ArrayRef<IndexExpr> lbs,
  ...
  void forExplicitParallelLoopIE(IndexExpr lb, IndexExpr ub,
      IndexExpr threadNum, NxLoopBodyFn builderFn) const;
  ...
  /*
     ...
     The loop body is constructed by calling each of the NxSimdIterateBodyFn
     ...
     To generate their output, each NxSimdIterateBodyFn function is given
  ...
  using NxSimdIterateBodyFn = impl::SimdIterateBodyFn<NxBuilder>;
  void simdIterateIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      mlir::ArrayRef<NxSimdIterateBodyFn> bodyBuilderFnList) const;
  ...
  /*
     ...
     here.
    */
  using NxSimdReductionBodyFn = impl::SimdReductionBodyFn<NxBuilder>;
  using NxSimdPostReductionBodyFn =
      impl::SimdPostReductionBodyFn<NxBuilder>;
  ...
  void simdReduceIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      mlir::ArrayRef<NxSimdReductionBodyFn> reductionBodyFnList,
      /* post reduction function (simd to scalar + post processing)*/
      mlir::ArrayRef<NxSimdPostReductionBodyFn> postReductionBodyFnList)
      const;
  ...
  */
  void simdReduce2DIE(IndexExpr lb, IndexExpr ub, int64_t VL, bool fullySimd,
      ...
      NxSimdReductionBodyFn reductionBodyFn,
      /* post reduction functions (post processing ONLY)*/
      NxSimdPostReductionBodyFn postReductionBodyFn) const;
  ...
  mlir::NxMovableOp movable() const;
...
//====--- Support for Affine Builder with Nx Mem Ops ------------------===//

// We use here a Affine builder that generates Nx Load and Store ops instead
// of the affine memory ops directly. This is because we can still generate
// Nx Ops while lowering the dialect, and the big advantage of the Nx
// memory operations is that they distinguish themselves if they are affine or
// not.
using AffineBuilderNxMem =
    GenericAffineBuilder<mlir::NxLoadOp, mlir::NxStoreOp>;

// =============================================================================
// IndexExpr Builder for building
// =============================================================================

struct IndexExprBuilderForNx : IndexExprBuilder {
  IndexExprBuilderForNx(mlir::Location loc) : IndexExprBuilder(loc) {}
  IndexExprBuilderForNx(mlir::OpBuilder &b, mlir::Location loc)
      : IndexExprBuilder(b, loc) {}
  IndexExprBuilderForNx(const DialectBuilder &db) : IndexExprBuilder(db) {}
  virtual ~IndexExprBuilderForNx() {}
...
// =============================================================================
// MultiDialectBuilder for Nx
// =============================================================================

// Recursive class specialized for AffineBuilderNxMem refereed to as
// affineKMem.
template <class... Ts>
struct MultiDialectBuilder<AffineBuilderNxMem, Ts...>
  ...
  AffineBuilderNxMem affineKMem;
};

// Recursive class specialized for NxBuilder referred to as nx.
template <class... Ts>
struct MultiDialectBuilder<NxBuilder, Ts...> : MultiDialectBuilder<Ts...> {
  MultiDialectBuilder(mlir::OpBuilder &b, mlir::Location loc)
...
// Recursive class specialized for IndexExprBuilderForNx referred to as
// denIE.
template <class... Ts>
struct MultiDialectBuilder<IndexExprBuilderForNx, Ts...>
    : MultiDialectBuilder<Ts...> {
  MultiDialectBuilder(mlir::OpBuilder &b, mlir::Location loc)
      : MultiDialectBuilder<Ts...>(b, loc), denIE(b, loc) {}
  MultiDialectBuilder(const DialectBuilder &db)
      : MultiDialectBuilder<Ts...>(db), denIE(db) {}
  IndexExprBuilderForNx denIE;
};
